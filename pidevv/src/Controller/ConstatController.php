...
public function indexfront(ConstatRepository $constatRepository,Request $request,PaginatorInterface $paginator,UserRepository $repository): Response

{   $connecteduser=$this->get('security.token_storage')->getToken()->getUser();//return connected user
    $user=$repository->find($connecteduser->getId());
    $donnees=$constatRepository->findByUser($user);
    $constats=$paginator->paginate(
        $donnees,// Requête contenant les données à paginer (ici les publications)
        $request->query->getInt('page',1),// Numéro de la page en cours, passé dans l'URL, 1 si aucune page
        3  // Nombre de résultats par page
    ...
    public function new(Request $request, ConstatRepository $constatRepository,UserRepository $userRepository): Response
    {
        $constat = new Constat();
